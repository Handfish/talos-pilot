...
impl TalosClient {
    ...
    pub async fn kubeconfig(&self) -> Result<String, TalosError> {
        ...
        let mut kubeconfig = Vec::new();
        while let Some(chunk) = stream.next().await {
            match chunk {
                Ok(data) => {
                    kubeconfig.extend_from_slice(&data.bytes);
                }
                Err(e) => {
        ...
        String::from_utf8(kubeconfig)
            .map_err(|e| TalosError::Connection(format!("Invalid UTF-8 in kubeconfig: {}", e)))
    }
